...
class Scrybs_Public {
	...
	public function __construct( $plugin_name, $version ) {
		...
		if(!empty($scrybs_settings)){
			...
			$this->prefloc_change_locale = $scrybs_settings[ 'prefloc_change_locale' ];    
			...
			/* Change to locale */
			if( $this->prefloc_change_locale ) 
			{    
				if ( ! is_admin() )
					add_filter('locale', array(&$this, 'prefloc_change_locale'));
			} 							
	...
	public function prefloc_change_locale ( $locale )
	{
		return get_locale_code($this->currentlang);
	}  	
