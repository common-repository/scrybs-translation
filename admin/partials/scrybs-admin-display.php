...
$prefloc_change_locale_checked = '';
$prefloc_change_locale = $scrybs->get_setting('prefloc_change_locale');
if (isset($prefloc_change_locale)) {
    if ($prefloc_change_locale == 'yes') {
        $prefloc_change_locale_checked = ' checked="checked"';
    }
} 
...
<div class="wrap">
	...
	<?php } ?>
    	<table class="form-table">
			...
			<tr>
				<th scope="row">
					<label for="prefloc_change_locale"><?php _e('Use Localisation files', $this->plugin_name); ?></label>
				</th>
				<td>
					<label><input type="checkbox" value="yes" id="prefloc_change_locale" name="<?php echo $this->plugin_name; ?>[prefloc_change_locale]"<?php echo $prefloc_change_locale_checked; ?>> <?php _e('Automatically use WordPress localisation files if available before translating.', $this->plugin_name); ?></label>
				</td>
			</tr> 
			<tr>
				<th scope="row">
					<label for="prefloc_download_locale"><?php _e('Download Localisation files', $this->plugin_name); ?></label>  <!-- $scrybs->get_active_languages() -->   
				</th>
				<td>  
					<button class="button button-primary" id="prefloc_download_locale" disabled=""><?php _e('Download', $this->plugin_name); ?></button>&nbsp;&nbsp;<button class="button button-secondary" id="prefloc_process_locale" disabled=""><?php _e('Show installed localisation', $this->plugin_name); ?></button>
					<br /><br /><em><?php _e('Automatically download the localisation files according the visitors choosen language before translating.', $this->plugin_name); ?></em>
					<div id="placeholder_prefloc_show_locale"></div>
				</td>
			</tr>
			<tr id="lang-sec-4">
				<th colspan="2">
