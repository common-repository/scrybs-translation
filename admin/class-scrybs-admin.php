...
class Scrybs_Admin {
	...
	public function scrybs_js_actions() { ?>
		...
		jQuery(document).ready(function($) {
			...
			jQuery('#prefloc_download_locale').prop('disabled', false);
			jQuery('#prefloc_download_locale').click(function(e) {
			...
			jQuery('#prefloc_process_locale').prop('disabled', false);
			jQuery('#prefloc_process_locale').click( function(e) {
				...
				var data = {
					'action': 'scrybs_action',
					'todo': 'prefloc_process_locale'
				};
				var btn = jQuery(this);    
				btn.prop('disabled', true); 
				var placeholder = jQuery('#placeholder_prefloc_show_locale');
				jQuery.post(ajaxurl, data, function(response) {    
					if(response == '1'){
						btn.find('i').remove();
						btn.prop('disabled', false);   
			     ...
			     var customHTML = '' + 			     
					'<div id="scrybs-modalbox" class="scrybsbox-blur">' +
							'<div class="scrybsbox"> ' +
								'<div class="scrybsclose-btn"><?php _e("Close", $this->plugin_name); ?> <i class="fa fa-times"></i></div> ' +
								'<h3 class="scrybsbox-title"><?php _e("WordPress Localisation files", $this->plugin_name); ?></h3>' +
								'<h4 class="scrybsbox-text"><?php _e("Languagepacks installed:", $this->plugin_name); ?></h4> ' +
								'<p class="scrybsbox-subtext"><?php echo $this->prefloc_show_locale(); ?></p> ' +
							'</div> ' +
					'</div> ';		
			...
			$('#placeholder_prefloc_show_locale').on('click', '.scrybsclose-btn', function() {
				$('#scrybs-modalbox').empty();
				
				$('#scrybs-modalbox').remove();
			});	
	...
	public function scrybs_action() {
		...
		}else if($todo == 'prefloc_download_locale'){
			if( $this->prefloc_download_locale() ){
				echo '1';
			}else{
				echo '0';
			}
		} else if($todo == 'prefloc_process_locale'){
			if( $this->prefloc_process_locale() ){
				echo '1';
			}else{
				echo '0';
			}
	...
	public function get_prefloc_languages_as_string( $string = true )
	{     
		// return as an array
		if( $string == false ) 
			return explode( ",", $this->l_target );	

		// return as a string, comma delimted		
		return $this->l_target;
	
	}
	
	public function prefloc_show_locale()
	{      
		global $scrybs;
		
		$available_languages = $scrybs->get_available_languages();	
		$selected_languages = $this->get_prefloc_languages_as_string(false);
		$word = "<span style=\"width: 100%;margin: 35px;\">";								
		foreach( $selected_languages as $code ){
			$word .= sprintf( "<span style=\"width: 33%%;float: left;text-align: center;font-size: 1rem;padding: 5px 0;\">%s (%s)</span>", $available_languages[$code], get_locale_code($code));    
		}      
		return $word . "</span>";  	
	}
	
	public function prefloc_process_locale()
	{     
		if( !empty( $this->l_target ) && is_string( $this->l_target ) )      
			return true; 
		else
			return false;			
	}
	
	public function prefloc_download_locale()
	{
		if( !empty( $this->l_target ) && is_string( $this->l_target ) ) :  
			
			$languages = $this->l_target; 
			...
			foreach( $languages as $language )
			{
			    ...
			    if ( !empty( $locale_code ) && !in_array( $locale_code, $languages ) ) 
				{
			        require_once( ABSPATH . 'wp-admin/includes/file.php' );
			        require_once( ABSPATH . 'wp-admin/includes/translation-install.php' );
			        
			        if ( false === wp_download_language_pack( $locale_code ) )
						$language_packs[$locale_code] = false; 
					else  					
						$language_packs[$locale_code] = true;  			        
			...
			if ( in_array( true, $language_packs ) )
				return true;
			else
				return false;
		...
		{	    										
			if($json['isvalid']=='yes'){
				if($json['error']==0){
					...
					if(isset($_POST['scrybs']['prefloc_change_locale']) && $_POST['scrybs']['prefloc_change_locale'] == 'yes'){
						$valid['prefloc_change_locale'] = 'yes';
					}else{
						$valid['prefloc_change_locale'] = 'no';
					}
